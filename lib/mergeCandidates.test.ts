...
describe('후보 병합', () => {
  it('개인 평점과 다른 그룹원의 평균을 분리합니다', () => {
    const restaurants = [{ placeId:'p1', name:'식당', category:'한식', lat:37, lng:127, googleRating:4, googleRatingsTotal:10, distanceMeters:50 }];
    const ratings = [
    ...
    expect(result.candidates[0]).toMatchObject({ personalRating:4, groupAverage:2 });
