...
따라서 회원가입 요청은 세션을 저장하지 않는 별도 Supabase 클라이언트에서 실행합니다. 기본
클라이언트의 익명 세션을 로그아웃하지 않으므로 가입 요청과 익명 세션 재생성이 경합하지 않고,
사용자는 인증 메일을 기다리는 동안에도 추천 기능을 계속 쓸 수 있습니다. 실사용자 세션에서는
회원가입 폼을 사용할 이유가 없으므로 가입 요청을 거부합니다.
...
않습니다. 인증 링크가 홈에서 열리면 기본 Supabase 클라이언트가 URL의 인증 정보를 처리해 기존
익명 세션을 실사용자 세션으로 교체합니다.

### 3.3 CAPTCHA는 익명 세션, 로그인, 가입 모두 적용합니다

Supabase Dashboard에서 Cloudflare Turnstile CAPTCHA 보호를 활성화하면 Auth 로그인과 가입
요청뿐 아니라 익명 가입 요청도 토큰을 요구합니다. 따라서 홈에 세션 없이 진입하면 공통 세션
게이트가 Turnstile 검증을 완료한 뒤 `signInAnonymously()`에 토큰을 전달합니다. 로그인 페이지에
직접 진입한 경우 추천용 익명 세션을 만들 필요가 없으므로 게이트를 생략합니다. 로그인 폼과 가입
폼도 각각 Turnstile 토큰을 API 요청에 포함합니다.
...
- `ensureSession()`: 세션이 없을 때 CAPTCHA 토큰으로 익명 세션 생성
- `signUp()`: 비영속 별도 Auth 클라이언트에서 이메일 가입과 홈 redirect 지정
...
`components/AnonymousSessionGate.tsx`

- 홈 등 추천 화면에 기존 세션이 없을 때만 Turnstile을 실행
- 검증 토큰으로 익명 세션을 생성할 때까지 자식 화면 대신 로딩 상태 표시
- `/login`에서는 익명 세션 생성을 건너뛰어 불필요한 이중 검증 방지
- 익명 세션 생성 실패 시 오류와 CAPTCHA 재시도 제공
...
- 익명 세션 생성 요청에 CAPTCHA 토큰 전달
- 기존 익명 세션을 유지한 채 비영속 클라이언트에서 가입
- 실사용자 세션에서는 가입 거부
...
- 신규 방문자의 익명 세션 게이트와 로그인·가입 폼에 Turnstile 표시
- CAPTCHA 전에는 제출 차단
- CAPTCHA 성공 후 익명 세션 생성·로그인·가입 요청 가능
...
- 익명 세션 생성, 로그인, 가입 모두 유효한 Turnstile 토큰을 요구합니다.
