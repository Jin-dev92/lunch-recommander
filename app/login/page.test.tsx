...
describe('로그인',()=>{
  ...
  it('이메일과 비밀번호 입력을 표시합니다',()=>{render(<LoginPage/>);expect(screen.getByLabelText('이메일')).toBeInTheDocument();expect(screen.getByLabelText('비밀번호')).toBeInTheDocument();expect(screen.getByRole('button',{name:'로그인'})).toBeInTheDocument();});
  ...
  it('로그인 실패 시 에러 메시지를 보여주고 이동하지 않습니다',async()=>{
    signInWithPassword.mockResolvedValue({error:{message:'잘못된 비밀번호입니다'}});
    render(<LoginPage/>);
    fillAndSubmit();
    await waitFor(()=>expect(screen.getByRole('alert')).toHaveTextContent('잘못된 비밀번호입니다'));
