...
- 현재 위치와 검색 반경(500m 또는 1km)을 기준으로 주변 음식점 한 곳을 추천합니다.
- 내 평점과 그룹 구성원들의 평균 평점을 함께 반영하는 하이브리드 평점을 사용합니다.
- 추천 카드에 음식점 이름, 카테고리, 거리를 한국어로 보여줍니다. 카테고리는 저장 키로 쓰는 기계값과 화면 표시용 한글 라벨을 분리해 다룹니다.
...
npx supabase functions deploy nearby signup-request approve-signup
```
